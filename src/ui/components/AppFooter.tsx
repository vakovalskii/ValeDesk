...
export function AppFooter({ workflowPanelOpen = false }: { workflowPanelOpen?: boolean }) {
  const { t } = useI18n();
  ...
  return (
    <div className={`fixed bottom-0 left-[280px] h-6 bg-surface-secondary/80 backdrop-blur-sm border-t border-ink-900/10 flex items-center justify-center px-4 text-xs text-muted z-10 ${workflowPanelOpen ? "right-[320px]" : "right-0"}`}>
